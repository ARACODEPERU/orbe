...
@section('content')
    ...
    <section class="page-banner bg-image pt-80 pb-80"
        data-background="{{ asset('themes/webpage/assets/images/banner/inner-banner.jpg') }}">
        <div class="container">
    ...
    <section class="product-area pt-30 pb-130">
        <div class="container">
            <div class="pb-20 bor-bottom shop-page-wrp d-flex justify-content-between align-items-center mb-65">
                <p class="fw-600">Mostrando del producto {{ $products->firstItem() }} al {{ $products->lastItem() }} de
                    {{ $count }} en Total</p>
                <div class="short">
                                    ...
                                    <div class="product__content">
                                        <h4 class="mb-15">
                                            <a class="primary-hover"
                                                href="{{ route('web_pro_description', $product->id) }}">
                                                {{ $product->name }}
                                            </a>
                                        </h4>
                                        <del>S/ {{ $product->price + $product->price * 0.2 }}</del><span
                                            class="primary-color ml-10">S/ {{ $product->price }}</span>
                                    </div>
                                    <a class="product__cart d-block bor-top" href="#0"
                                        onclick="agregarAlCarrito({ id: {{ $product->id }}, nombre: '{{ $product->name }}', precio: {{ $product->price }}, quantity: 1 })">

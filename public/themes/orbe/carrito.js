...
function actualizarContador(valor) {
    ...
    let carrito_o = JSON.parse(localStorage.getItem('carrito'));

    // Verificar si hay datos en el carrito
    if (carrito_o && carrito_o.length > 0) {
        // Calcular el total de los precios
        let total = carrito_o.reduce((acc, producto) => acc + producto.precio, 0);
        var totalhtml = document.getElementById('totalCarritoWeb');
        totalhtml.innerHTML = 'S/ ' + total.toFixed(2);
    } else {
